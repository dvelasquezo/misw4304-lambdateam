...
class VistaBlacklists(Resource):
    def post(self):
        ...
        nuevo_email = EmailListaNegra(email=request.json["email"], motivo=request.json["motivo"],
                                ip=request.json["ip"], id_app=request.json["id_app"], fecha_solicitud=datetime.now())
    ...
    def get(self):
       ...
       if email_consultado != None:
           return "El email esta en la lista negra", 200
